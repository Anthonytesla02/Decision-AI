...
class DecisionSplitterApp {
    ...
    initializeEventListeners() {
        ...
        // Paystack payment buttons (use delegation for dynamic content)
        document.addEventListener('click', (e) => {
            if (e.target.classList.contains('paystack-btn') || e.target.closest('.paystack-btn')) {
                e.preventDefault();
                const button = e.target.classList.contains('paystack-btn') ? e.target : e.target.closest('.paystack-btn');
                this.handlePaystackPayment(button);
            }
        });
    }
...
Trust your analysis, but also trust your intuition. If you've done the logical work and still feel uncertain, listen to what your gut is telling you about which path feels more authentic and energizing.`;
    ...
    async handlePaystackPayment(button) {
        // Check if user is logged in
        if (!this.currentUser) {
            this.showToast('Please log in to upgrade your plan', 'error');
            this.showPage('auth');
            return;
        }
        ...
        // Get plan code and amount in kobo
        const planConfig = this.getPaystackPlanConfig(plan, amount);
        
        // Get Paystack public key
        const publicKey = await this.getPaystackPublicKey();
        if (!publicKey) {
            this.showToast('Payment system is not configured. Please contact support.', 'error');
            return;
        }
        ...
        console.log('Initiating payment with:', { 
            ...
            amountInGHS: planConfig.amount / 100,
            currency: 'GHS',
            email: this.currentUser.email,
            publicKey: publicKey.substring(0, 20) + '...'
        });

        // Initialize Paystack payment - using one-time payment instead of subscription
        const handler = PaystackPop.setup({
            key: publicKey,
            email: this.currentUser.email,
            amount: planConfig.amount, // Amount in pesewas
            currency: 'GHS',
            ref: 'dec_' + Date.now(),
            metadata: {
                plan: plan,
                ...
                custom_fields: [
                    ...
                    },
                    {
                        ...
                        value: this.currentUser.id.toString()
                    }
                ]
            },
            callback: (response) => {
                console.log('Payment successful:', response);
                this.handlePaymentSuccess(response, plan, planName);
            },
            onClose: () => {
    ...
    getPaystackPlanConfig(plan, usdAmount) {
        // Map plan names to your actual Paystack plan codes with Ghana cedis amounts
        const planMappings = {
            'basic': {
                code: 'PLN_nzh240uvvavt1wr',
                amount: 200 // 2 cedis in pesewas (2 * 100)
            },
            'professional': {
                code: 'PLN_sh1x79iqqxvj20m', 
                amount: 10000 // 100 cedis in pesewas (100 * 100)
            },
            'premium': {
                code: 'PLN_e3cd7974vply6c1',
                amount: 25000 // 250 cedis in pesewas (250 * 100)
            },
            'enterprise': {
                code: 'PLN_e3cd7974vply6c1',
                amount: 25000 // 250 cedis in pesewas (250 * 100)
    ...
    async getPaystackPublicKey() {
        ...
        console.error('No Paystack public key found');
        return null;
    }

    async handlePaymentSuccess(response, plan, planName) {
        try {
            this.showLoadingOverlay();
            
            // Verify payment on the server (in a real app, this would be a server call)
            const paymentVerified = await this.verifyPayment(response.reference);
            
            if (paymentVerified) {
                // Update user's plan and credits
                await this.updateUserPlan(plan, planName);
                
                this.showToast(`Successfully upgraded to ${planName} plan!`, 'success');
                this.showPage('dashboard');
            } else {
            ...
            this.showToast('Payment processing failed. Please contact support.', 'error');
        } finally {
            this.hideLoadingOverlay();
        }
    }
    ...
    async updateUserPlan(plan, planName) {
        // Update user's plan and credits in localStorage
        const users = this.getUsersFromStorage();
        const userIndex = users.findIndex(u => u.id === this.currentUser.id);
        ...
        if (userIndex !== -1) {
            ...
            if (plan === 'basic' || plan === 'professional') {
                users[userIndex].credits = 50; // 50 credits for basic/professional
            } else if (plan === 'premium' || plan === 'enterprise') {
                users[userIndex].credits = 999; // Unlimited (represented as 999)
            }
            
            // Save updated users
            localStorage.setItem('decision_app_users', JSON.stringify(users));
